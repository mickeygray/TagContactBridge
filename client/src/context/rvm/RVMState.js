...
import RVMContext from "./rvmContext";
import rvmReducer from "./rvmReducer";

const RVMState = (props) => {
  const initialState = {
    txtFiles: [], // 🟠 Raw uploaded TXT files
    scrapingQueue: [], // 🟡 TXT files currently being scraped
    extractedLeads: [], // 🟢 Leads successfully extracted from TXT files
    scrubbedLeads: [], // 🔵 Leads after DNC scrubbing
  };

  const [state, dispatch] = useReducer(rvmReducer, initialState);
  ...
  const uploadFileToCase = async ({ file, caseID }) => {
    try {
      ...
      await axios.post("/api/rvms/uploadDocument", formData, {
        headers: {
  ...
  const scrubLeads = async () => {
    try {
      const res = await axios.post("/api/rvms/scrub", {
  ...
  return (
    <RVMContext.Provider
      value={{
        txtFiles: state.txtFiles,
        scrapingQueue: state.scrapingQueue,
        extractedLeads: state.extractedLeads,
        scrubbedLeads: state.scrubbedLeads,
        uploadTXTs,
        removeFile,
        processBatch,
        uploadFileToCase,
